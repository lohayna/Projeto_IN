...
for trace in range(total_receivers):
    ...
    cmp_gather[region, trace] += 0.2*ground_roll_amplitude 

    cmp_gather[:,trace] *= time[::-1]**1.5
    
    cmp_gather[:,trace] *= offset[trace]**(-0.1) if offset[trace] != 0.0 else 0.9

file = f"cmp_gather_{time_samples}x{total_receivers}.bin"

cmp_gather.flatten("F").astype(np.float32, order = "F").tofile(file) 